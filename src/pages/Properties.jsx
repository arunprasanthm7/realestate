...
export function Properties(){
    ...
    return( <div id="propertyContainer" className='grid grid-cols-4 gap-20 '>
    {propsData.map((props)=>{
       return( 
       <div className='relative'>
        <img src={props.image} className="w-[280px] h-[200px] rounded-lg " />
        <img className="w-5 h-5 absolute top-6 right-6 z-10 cursor-pointer" id={props.id} onClick={()=>{savedProps(props.id)}} src={saveEmpty} title='save'/>
        <div className='px-[22px] py-3 w-[240px] h-[110px] absolute left-[20px] bottom-[-52px] text-[12px]   shadow-md bg-white rounded-lg z-10'>
